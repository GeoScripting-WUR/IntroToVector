...
author: "Jan Verbesselt, Sytze de Bruyn, Loïc Dutrieux, Valerio Avitabile"
...
- How to read/write data and change data format with **rgdal** package (`writeOGR()` and `writeOGR()`)
- Visualize spatial vector data in R and in GoogleEarth
- How to perform simple operations on Geometries in R using the **rgeos** package


# Excercise of today

First download and unzip the following "manually" files in your `data` directory:
https://www.dropbox.com/s/w25dwfo7c8wuaxv/read-write-shapefiles.zip?dl=0

Second, create a clear and documented script that:

- Reads in the rivers, centroids, counties shapefiles using the `rgdal` library 
- Creates a map that visualises the rivers, counties, and centroids at once (hint use `plot(, add = TRUE)`)
- Write the river file a kml files, to check where the river file is located by opening the kml in Google Earth
- Create a buffer around the rivers file and check which centroids fall within the buffer.

Third answer the questions as comments within the script:
