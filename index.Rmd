...
output:
  knitrBootstrap::bootstrap_document:
    title: "Week 3, Lesson 6: Intro to vector with R"
    theme: cosmo
    menu: FALSE
---
...
Authors: Jan Verbesselt, Sytze de Bruyn, Loïc Dutrieux, Valerio Avitabile
...
The GDAL library is well-documented (http://gdal.org/), but with a catch for R and Python programmers. The GDAL (and associated OGR) library and command line tools are all written in C and C++. Bindings are available that allow access from a variety of other languages including `R` and `Python` but the documentation is all written for the C++ version of the libraries. This can make reading the documentation rather challenging. Fortunaly, the `rgdal` package, providing GDAL bindings in R, is well documented with lots of examples. The same is valid for the Python libaries.
...
# note some problems were report with this step see Q&A
...
Second, create a clear and documented script as part of a new Rstudio Project (set-up as descriped in lesson 4) that:
...
## Submit
Put your project on your gitHub account as a new repository post the ***clone url*** in the forum before tommorow (1030)
